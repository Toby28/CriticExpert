...
class Llama_70b_2:
    ...
    def run(self,all_set, all_label, train_set, train_label, test_set, test_label,target_file):
        ...
        index2label = ["Neoplasms", "Digestive system diseases", "Nervous system diseases", "Cardiovascular diseases", "General pathological conditions"]
        ...
        # for i in tqdm.tqdm(range(8000)):
            ...
            # print(index)
            prompt += f"1. {all_set[index[0][0]][:-2]} -> {index2label[all_label[index[0][0]]-1]}\n"
            prompt += f"2. {all_set[index[0][1]][:-2]} -> {index2label[all_label[index[0][1]]-1]}\n"
            prompt += f"3. {all_set[index[0][2]][:-2]} -> {index2label[all_label[index[0][2]]-1]}\n"
            prompt += "\n"
            prompt += "Query:\n" + test_set[i]
...
def main():
    ...
    target_file = f"medical_gpt_v4_test2.json"

    # llm.run(table, label, X_train, y_train, X_test, y_test, target_file)
    llm.run(table, label, None, None, x_test, y_test, target_file)
