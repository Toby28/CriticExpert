...
from models.roberta import RobertlargeDataset, RobertalargeClass, RobertDataset, RobertaClass, distilbertClass, distilbertDataset
...
def main(args):
    ...
    if dataset=="mamual":
        ...
        # new_df = full_data_preprocess("res/mamual_gpt3.json")
        medical = mamual()
        if llm=="llama":
    ...
    if method_name.lower() == 'roberta-large'.lower():
        ...
        dropout = 0.3
        teacher_model = RobertalargeClass(dropout, outclass)
        learning_rate = 1E-5
        # learning_rate = 1E-6
        ...
        validation_set = RobertlargeDataset(val, MAX_LEN)
        dropout = 0.1
        teacher_model = distilbertClass(dropout, outclass)
        learning_rate = 1E-5
        # optimizer = torch.optim.RMSprop(params=model.parameters(), lr=learning_rate)
    ...
    model_file = f"./train2/method_name-roberta-large-dataset-{dataset}-llm-{llm}-epoches-50_v2.bin"
    state_dict = torch.load(model_file, map_location=device)
    # teacher_model.load_state_dict(state_dict)
    # teacher_model = torch.load(model_file, weights_only=True, map_location=device)
    teacher_model.load_state_dict(state_dict)
    teacher_model.eval()
    ...
    student_config = RobertaConfig(
        hidden_size=768,  # smaller than 768, 384
        num_attention_heads=16,
        num_hidden_layers=12
    )
    '''
    student_model = distilbertClass(0.1, outclass)
    ...
    for epoch in range(100):
        ...
        for data in tqdm(train_dl):
            ...
            with torch.no_grad():
                teacher_outputs, output2 = teacher_model(ids, mask, token_type_ids)
            
            student_outputs1, student_outputs2 = student_model(ids, mask, token_type_ids)
            
            
            loss = distillation_loss(student_outputs1, teacher_outputs, targets1) + distillation_loss(student_outputs2, output2, targets2)
            
            tr_loss += loss.item()
            big_val1, big_idx1 = torch.max(student_outputs1, dim=1)
            n_correct1 += calcuate_accuracy(big_idx1, targets1)
        ...
        with torch.no_grad():
            ...
            for data in tqdm(val_dl):
                ...
                mask = data['mask'].to(device, dtype=torch.long)
                token_type_ids = data['token_type_ids'].to(device, dtype=torch.long)
                outputs1, output2 = student_model(ids, mask, token_type_ids)
                # outputs1 = outputs1.squeeze()
                ...
                big_val1, big_idx1 = torch.max(outputs1, dim=1)
                n_correct1 += calcuate_accuracy(big_idx1, targets1, flag)
    ...
    filename = f"method_name-distil-dataset-{dataset}-llm-{llm}-flag-{flag}-epoches-{epoches}_dist.json"
    ...
    # json.save(logfilename, res)

    #output_model_file = f"distil3.bin"
    # output_model = "./train2/" + output_model_file
    # output_vocab_file = './'

    # torch.save(student_model, output_model)
