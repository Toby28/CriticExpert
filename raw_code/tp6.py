...
def main(args):
    ...
    filename = f"method_name-{method_name}-dataset-{dataset}-llm-{llm}-flag-{flag}-epoches-{epoches}_v3.npy"
    ...
    if method_name.lower() == 'roberta-large'.lower():
        ...
        training_set = distilbertDataset(train, MAX_LEN)
        validation_set = distilbertDataset(val, MAX_LEN)
    ...
    for epoch in range(EPOCHS):
        ...
        for data in tqdm(train_dl):
            ...
            # print("min:", ids.min().item(), "max:", ids.max().item())
    ...
    # output_model_file = f"method_name-{method_name}-dataset-{dataset}-llm-{llm}-epoches-{epoches}_v3.bin"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--llm', default='mixtral', choices=['llama', "mixtral", "chatgpt"])
