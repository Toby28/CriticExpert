...
class mamual:
    def __init__(self):
        ...
        self.label2index = {"Unlabeled Praise": 0, "Labeled Praise": 1,
                            "Reflection": 2, "Behavior": 3,
                            "Information Question": 4,
                            "Descriptive Question": 5,
                           "Indirect Command":6, "Direct Command":7,
                           "Negative Talk":8, "Neutral Talk":9
                            }
            ...
            if dialogue["DPICS_Code"][i] is not None:
                ...
                if dialogue['DPICS_Code'][i] == dialogue['test_label'][i]:
                    rows.append({"text": tp_prompt,
                                 "pred": dialogue['DPICS_Code'][i],
                                 "labels1": 1,
                                 "labels2": int(dialogue['test_label'][i])
                                 })
                else:
                    rows.append({"text": tp_prompt,
                                 "pred": dialogue['DPICS_Code'][i],
...
class pcit:
    def __init__(self):
        ...
        self.label2index = {"Unlabeled Praise": 0, "Labeled Praise": 1,
                            "Reflection": 2, "Behavior": 3,
                            "Question": 4, "Command":5, "Negative Talk":6, "Neutral Talk":7}
            ...
            if dialogue["MED_Code"][i] is not None and dialogue['MED_Code'][i] in self.label2index.keys():
              # if len(data[i]) <500:
                ...
                if self.label2index[dialogue['MED_Code'][i]] == data_label[i] - 1:
                    rows.append({"text": tp_prompt,
                                 "pred": self.label2index[dialogue['MED_Code'][i]],
                                 "labels1": 1,
...
class mental_dataset:
    ...
    def get_llama_test_results(self):
        val_ds = self.load_res('./clean/mental_gpt_v2_test_clean.json', flag='test')
        return val_ds

    def get_mixtral_test_results(self):
        val_ds = self.load_res('./clean/mental_gpt_v4_test_clean.json', flag='test')
        return val_ds

    def get_chatgpt4o_test_results(self):
        val_ds = self.load_res('./clean/mental_gpt_v6_test_clean.json', flag='test', start=0)
        return val_ds
        ...
        for i in range(len(dialogue['MED_Code'])):
            if dialogue["MED_Code"][i] is not None and dialogue['MED_Code'][i] in self.text_map.keys():
                ...
                if self.text_map[dialogue['MED_Code'][i]] == data_label[i]:
                    rows.append({"text": tp_prompt,
                                 "pred": self.text_map[dialogue['MED_Code'][i]],
                                 "labels1": 1,
                                 "labels2": int(data_label[i])
                                 })
                else:
                    rows.append({"text": tp_prompt,
                                 "pred": self.text_map[dialogue['MED_Code'][i]],
    ...
    def load_res(self, path, flag=None, start = 0):
        if flag == "train":
            data = self.train[start:]
            data_label = self.train_label[start:]
            with open(path, 'r') as file:
        ...
        for i in range(len(dialogue['MED_Code'])):
            if dialogue["MED_Code"][i] is not None and dialogue['MED_Code'][i] in self.text_map.keys():
                rows.append({"text": self.text_map[dialogue['MED_Code'][i]],
                             "labels2": int(data_label[i])
                             })
