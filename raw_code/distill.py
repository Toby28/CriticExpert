...
def main(args):
    ...
    # torch.save(model.state_dict(), 'model_file.bin')
    ...
    for epoch in range(50):
        ...
        for data in tqdm(train_dl):
            ...
            student_outputs = student_model(ids, mask, token_type_ids)
            
            loss = distillation_loss(student_outputs.logits, teacher_outputs, targets1)
            
            tr_loss += loss.item()
            big_val1, big_idx1 = torch.max(student_outputs.logits, dim=1)
            n_correct1 += calcuate_accuracy(big_idx1, targets1)
        ...
        with torch.no_grad():
            ...
            for data in tqdm(val_dl):
                ...
                mask = data['mask'].to(device, dtype=torch.long)
                token_type_ids = data['token_type_ids'].to(device, dtype=torch.long)
                outputs1 = student_model(ids, mask, token_type_ids)
                # outputs1 = outputs1.squeeze()
                ...
                big_val1, big_idx1 = torch.max(outputs1.logits, dim=1)
                n_correct1 += calcuate_accuracy(big_idx1, targets1, flag)
    ...
    filename = f"method_name-{method_name}-dataset-{dataset}-llm-{llm}-flag-{flag}-epoches-{epoches}_dist.json"
    ...
    # output_model_file = f"method_name-{method_name}-dataset3-{dataset}-group-{kkk}-epoches-{epoches}.bin"
